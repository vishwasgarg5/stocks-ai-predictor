"""GitHub-only NIFTY 50 incremental prediction workflow."""
...
def run():
    ...
    ledger = read_ledger(PREDICTIONS_CSV.read_text() if PREDICTIONS_CSV.exists() else "")

    # Evaluate only predictions whose target date has actually passed.
    actuals = {}
    latest_market_date = max(df.index.max() for df in raw.values())
    ...
    target_date = str(latest_market_date.date())
    # A prediction is unique by symbol + target session. Existing evaluated history is never overwritten.
    pending = set(ledger.loc[ledger["actual_close"].isna(), "symbol"].astype(str))
    for _, row in top5.iterrows():
        symbol = str(row.symbol)
        if symbol in pending:
            continue
        p = predict_next(symbol, raw[symbol], features[symbol], models)
        ledger = save_prediction(ledger, p, int(row["rank"]), float(row["score"]), target_date=target_date)
