...
def test_config_is_stage28():
    ...
    assert PRICE_BUCKET_NAMES==["<10","10-49","50-99","100-249","250-499","500-999","1000-2499",">2500"] and MAX_PER_PRICE_BUCKET==6 and FINAL_BEST_PER_BUCKET==1
...
def test_price_buckets_are_current():
    from src.stage4_engine import price_bucket,price_bucket_code
    assert [price_bucket(x) for x in [3000,1500,750,300,150,75,25,9]]==[">2500","1000-2499","500-999","250-499","100-249","50-99","10-49","<10"]
    assert [price_bucket_code(x) for x in [3000,1500,750,300,150,75,25,9]]==["B1","B2","B3","B4","B5","B6","B7","B0"]
...
def test_portfolio_engine_prediction_chain():
    ...
    pred=pd.DataFrame([{"Symbol":"TEST.NS","Pred_Close":120,"Pred_Open":118,"Pred_High":123,"Pred_Low":116,"Confidence":85,"Action":"BUY"}]);base=pd.DataFrame([{"Stock":"TEST.NS","Ticker":"TEST.NS","Quantity":100.,"Average_Price":100.,"Reported_PnL":np.nan,"Reported_Return":np.nan}])
    import src.portfolio_report as pr;old=pr._latest_predictions;pr._latest_predictions=lambda:(pred,"2026-09-04")
    try:
        out,date=_attach_predictions(base);assert date=="2026-09-04" and float(out.loc[0,"Pred_Close"])==120 and float(out.loc[0,"Pred_High"])==123;row=pr._plan_row(base.iloc[0],pred.iloc[0],date);assert round(float(row["Profit_Target_Price"]),10)==110 and float(row["Projected_Return_At_AI_Target"])==20
...
def test_morning_report_includes_buckets_and_portfolio_sections():
    ...
    d=pd.DataFrame([{"Symbol":"TEST","PriceBucket":"100-249","Current_Price":100,"Pred_Close":108,"Expected_Return":8,"Confidence":80,"FinalDecisionScore":80,"Action":"BUY","Horizon_1D":3,"Horizon_5D":7,"Horizon_20D":12}]);report=morning_report("2026-09-07","2026-09-04",d,pd.DataFrame(),pd.DataFrame(),accuracy={"PreviousAccuracy":70,"CurrentAccuracy":72},scan={"Universe":100,"Data":90,"Liquid":80,"AI":40,"Selected":1},portfolio={"Positions":1,"Value":10000,"PnL":500,"Return":5,"Rows":[{"Stock":"TEST","Quantity":10,"Decision":"HOLD","Current_Price":"₹100","Average_Price":"₹95","Profit_Target":"₹104.50","Sell_Window":"2026-09-10"}]})
...
def test_morning_report_sections_are_explicitly_separated_and_ordered():
    ...
    d=pd.DataFrame([{"Symbol":f"S{i}","PriceBucket":b,"Current_Price":20+i*500,"Pred_Close":21+i*500,"FinalDecisionScore":80-i,"Action":"BUY","Horizon_1D":1,"Horizon_5D":2,"Horizon_20D":3} for i,b in enumerate(buckets)]);report=morning_report("2026-09-07","2026-09-04",d,pd.DataFrame(),pd.DataFrame(),scan={},accuracy={},portfolio={});sections=[x.strip() for x in report.split("§§TELEGRAM_SECTION§§") if x.strip()];assert "MARKET OVERVIEW" in sections[1];bucket_order=["10-49","50-99","100-249","250-499","500-999","1000-2499",">2500"];positions=[next(i for i,s in enumerate(sections) if f"₹ {b}" in s) for b in bucket_order];assert positions==sorted(positions);assert len(_report_messages(report))>=1;assert any("BEST PICK" in s for s in sections);assert any("PREDICTED OHLC" in s for s in sections);assert any("MULTI-HORIZON OUTLOOK" in s for s in sections);assert any("AI PORTFOLIO MANAGER" in s for s in sections)
...
def test_nan_market_values_are_hidden():
    ...
    rows=_market({"FINNIFTY":{"Close":np.nan,"Change1D":np.nan}},"SIDEWAYS");assert "nan" not in "\n".join(rows).lower()
def test_telegram_html_escaping_and_formatting():
    from src.telegram_report import _markdown_to_telegram_html
    source="📈 *AI NSE MORNING REPORT*\nABC_TEST.NS [WATCH] -2.5% | ₹123.45\n```\nA_B [C] <D>\n```";rendered=_markdown_to_telegram_html(source);assert "<b>AI NSE MORNING REPORT</b>" in rendered and "ABC_TEST.NS [WATCH] -2.5% | ₹123.45" in rendered and "<pre>A_B [C] &lt;D&gt;</pre>" in rendered and "<pre>" in rendered and "```" not in rendered
