...
FORBIDDEN_LEGACY_PATHS=[".github/workflows/stage2_morning.yml",".github/workflows/stage2_evening.yml",".github/workflows/stage2_weekly.yml",".github/workflows/morning_prediction.yml",".github/workflows/evening_evaluate_retrain.yml",".github/workflows/weekly_report.yml",".github/workflows/test_stage2.yml",".github/workflows/stage4_tests.yml","main.py","morning.py","stage15_morning.py","config.py","weekly_report.py","evening.py","src/stage15.py","src/ranking.py","models/champion.pkl","reports/performance.csv","reports/weekly_report.csv","tests/test_stage2.py"]
...
def test_selection_returns_all_qualified_without_global_five_cap():
    ...
    rows=[{"Symbol":f"S{i}","TechnicalScore":90,"Expected_Return":5,"Confidence":90,"Direction_Confidence":90,"Direction":"UP","PriceBucket":b} for i,b in enumerate([">1000","500-999","100-499","50-99","10-49"])]
    r=select_top_stocks(pd.DataFrame(rows),top_n=None); assert len(r)==5 and set(r.Symbol)=={f"S{i}" for i in range(5)}
def test_price_bucket_limit_is_configurable():
    from src.selection import select_top_stocks
    rows=[{"Symbol":f"B1{i}","PriceBucket":"B1","TechnicalScore":95-i,"Expected_Return":5,"Confidence":90,"Direction_Confidence":85,"Direction":"UP"} for i in range(4)]
    r=select_top_stocks(pd.DataFrame(rows),top_n=None,max_per_bucket=2); assert len(r)==2
