...
REQUIRED_SOURCE_FILES=[
    "src/__init__.py","src/config.py","src/features.py","src/market_data.py","src/models.py","src/prediction.py",
    "src/multihorizon.py","src/selection.py","src/stage4_engine.py","src/stage45_engine.py","src/final_intelligence.py",
    "src/evaluation.py","src/retraining.py","src/ledger.py","src/morning_runner.py","src/evening.py","src/telegram_report.py",
    "src/weekly_report.py","src/portfolio_report.py"
]
REQUIRED_WORKFLOWS=[
    ".github/workflows/stage10_4_morning.yml",".github/workflows/stage10_4_evening.yml",".github/workflows/stage10_4_weekly.yml"
]
FORBIDDEN_LEGACY_PATHS=[
    ".github/workflows/stage2_morning.yml",".github/workflows/stage2_evening.yml",".github/workflows/stage2_weekly.yml",
    ".github/workflows/morning_prediction.yml",".github/workflows/evening_evaluate_retrain.yml",".github/workflows/weekly_report.yml",
    ".github/workflows/test_stage2.yml",".github/workflows/stage4_tests.yml","main.py","morning.py","stage15_morning.py",
    "config.py","weekly_report.py","evening.py","src/stage15.py","src/ranking.py","models/champion.pkl",
    "reports/performance.csv","reports/weekly_report.csv"
]
CORE_MODULES=[
    "src.config","src.features","src.market_data","src.models","src.prediction","src.multihorizon","src.selection",
    "src.stage4_engine","src.stage45_engine","src.final_intelligence","src.evaluation","src.retraining","src.ledger",
    "src.morning_runner","src.evening","src.telegram_report","src.weekly_report","src.portfolio_report"
]
...
def test_legacy_workflows_are_removed():
...
def test_exactly_three_production_workflows():
    ...
    assert workflows==["stage10_4_evening.yml","stage10_4_morning.yml","stage10_4_weekly.yml"]
...
def test_config_is_stage10_4():
    ...
    assert MODEL_VERSION.startswith("stage10.4")
    assert STAGE_NAME.startswith("Stage 10.4")
    assert HISTORY_PERIOD=="4mo"
    assert TOP_N is None
    assert MAX_UNIVERSE==0
    assert tuple(MULTI_HORIZONS)==(1,3,5,7,20)
...
def test_selection_returns_all_qualified_without_global_five_cap():
    ...
    rows=[]
    for i,b in enumerate([">1000","500-999","100-499","50-99","10-49"]):
        rows.append({"Symbol":f"S{i}","TechnicalScore":90,"Expected_Return":5,"Confidence":90,"Direction_Confidence":90,"Direction":"UP","PriceBucket":b})
    r=select_top_stocks(pd.DataFrame(rows),top_n=None)
    assert len(r)==5 and set(r.Symbol)=={f"S{i}" for i in range(5)}

def test_price_bucket_limit_is_configurable():
    from src.selection import select_top_stocks
    rows=[]
    for i in range(4): rows.append({"Symbol":f"B1{i}","PriceBucket":"B1","TechnicalScore":95-i,"Expected_Return":5,"Confidence":90,"Direction_Confidence":85,"Direction":"UP"})
    r=select_top_stocks(pd.DataFrame(rows),top_n=None,max_per_bucket=2)
    assert len(r)==2 and r["PriceBucket"].value_counts().max()==2
...
def test_direction_return_conflict_reduces_trade_confidence():
    ...
    aligned=calculate_trade_confidence({**base,"Expected_Return":5}); conflict=calculate_trade_confidence({**base,"Expected_Return":-5})
...
def test_prediction_interval_ordering():
    from src.final_intelligence import _num
    base=100.;u=5.;spread=base*u/100
    assert _num(base-spread)<=_num(base)<=_num(base+spread)
...
def test_final_action_guardrails():
    ...
    assert final_action(safe,"BULL")=="BUY"
    assert final_action({**safe,"PredictionUncertaintyPct":20},"BULL")=="NO TRADE"

def test_final_manifest_is_stage10_4():
    from src.final_intelligence import final_stage_manifest
    m=final_stage_manifest();assert m["Stage"]=="10.4";assert set(m["SubStages"])=={"5","6","7","8","9","10"}
...
def test_portfolio_engine_has_sell_and_average_logic():
    ...
    out=_average_plan(row.copy());assert out["Decision"] in {"AVG","HOLD / RECOVERY","HOLD"};assert out["Profit_Target_Price"]==110.

def test_morning_report_includes_buckets_and_portfolio_sections():
    from src.telegram_report import morning_report
    d=pd.DataFrame([{"Symbol":"TEST","PriceBucket":"100-499","Current_Price":100,"Pred_Close":108,"Expected_Return":8,"Confidence":80,"FinalDecisionScore":80,"Action":"BUY","Horizon_1D":3,"Horizon_5D":7,"Horizon_20D":12}])
    ...
    assert "100-499" in report and "BEST PICK" in report and "AI PORTFOLIO MANAGER" in report and "Sell Window" in report
