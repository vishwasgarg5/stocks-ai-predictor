...
# =============================================================================
# STAGE 4 TEST SUITE — protects Stage 2 + Stage 3A + Stage 3B + Stage 4.
# Easy access: Stage 3B horizon checks are at the bottom of this file.
# =============================================================================
...
def test_requirements_and_readme_exist():
    assert (ROOT/"requirements.txt").exists() and (ROOT/"README.md").exists()

def test_next_session_target_alignment():
    df=pd.DataFrame({"Open":[10,11,12],"High":[11,12,13],"Low":[9,10,11],"Close":[10.5,11.5,12.5]})
    for c in ["Open","High","Low","Close"]: df[f"Target_{c}"]=df[c].shift(-1)
    assert df.loc[0,"Target_Open"]==11 and df.loc[0,"Target_Close"]==11.5 and pd.isna(df.loc[2,"Target_Close"])
...
def test_prediction_values_are_finite(): assert np.isfinite(np.array([100.,105.,98.,103.])).all()
...
def test_stage3b_horizon_targets_are_future_only():
    ...
    for h in [1,3,5]:
        target=close.shift(-h); assert target.iloc[0]==close.iloc[h]
        assert pd.isna(target.iloc[-h:]).all()
