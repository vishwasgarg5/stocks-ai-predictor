...
def test_two_stocks_max_per_price_bucket():
    ...
    r=select_top_stocks(pd.DataFrame(rows),10); assert len(r)==4 and r["PriceBucket"].value_counts().max()==2
...
def test_stage4_5_weights_sum_to_one(): assert abs(.16+.14+.14+.11+.07+.09+.09+.10+.10-1)<1e-9

def test_stage45_uncertainty_score_decreases_with_uncertainty():
    from src.stage45_engine import add_prediction_uncertainty
    d=pd.DataFrame([{"Symbol":"AAA","DataCutoff":"2026-09-02","Confidence":80}])
    assert "PredictionUncertaintyPct" in add_prediction_uncertainty(d,{"AAA":pd.DataFrame()},{}).columns
