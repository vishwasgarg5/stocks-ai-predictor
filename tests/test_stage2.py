...
# ============================================================
# Make repository root importable in GitHub Actions
# ============================================================
...
def _import_optional(module_name: str):
    """
    Import a module without crashing the whole test suite.

    Tests that genuinely require the module can skip when it is
    unavailable. The separate diagnostic test below exposes the
...
def _import_required(module_name: str):
    """
    Import a required production module and expose the real error.
    """
    try:
        return importlib.import_module(module_name)
...
def test_stage2_module_import_diagnostics():
    """
    Import every core Stage 2 module.

    This test deliberately does not skip import failures.

    If a production module has a real problem, GitHub Actions
    will show the exact module and exception.
    """
...
def test_next_day_target_alignment():
    ...
    assert pd.isna(
        df.loc[3, "Target_Open"]
    )

    assert pd.isna(
        df.loc[3, "Target_Close"]
    )
...
def test_no_current_day_target_leakage():
    ...
    assert (
        df.loc[0, "Target_Close"]
        != df.loc[0, "Close"]
    )

    assert (
        df.loc[1, "Target_Close"]
        != df.loc[1, "Close"]
    )
...
def test_feature_engineering_smoke():
    ...
    features = _import_optional(
        "src.features"
    )

    if features is None:
        pytest.skip(
            "src.features unavailable"
        )
    ...
    if function is None:
        pytest.skip(
            "No recognised feature "
            "engineering function found"
        )
...
def test_models_module_contract():
    ...
    models = _import_optional(
        "src.models"
    )

    if models is None:
        pytest.skip(
            "src.models unavailable"
        )
...
def test_no_sqlite_dependency_in_repository():
    """
    Stage 2 must not use a local SQLite database.

    This test checks for actual SQLite imports or connection
    calls. It deliberately ignores this test file itself so
    that mentioning SQLite here does not cause a false failure.
    """
    ...
    for path in ROOT.rglob("*.py"):
        ...
        for line_number, line in enumerate(
            ...
            actual_sqlite_usage = (
                ...
                or "sqlite3.connect(" in stripped
                or "sqlite3.Connection" in stripped
            )

            if actual_sqlite_usage:
...
def test_synthetic_next_day_flow():
    ...
    assert (
        last_target["Target_Open"]
        == 104
    )

    assert (
        last_target["Target_High"]
        == 109
    )

    assert (
        last_target["Target_Low"]
        == 102
    )

    assert (
        last_target["Target_Close"]
        == 107
    )
...
def test_features_module_available():
...
def test_stage2_high_level_contract():
    """
    High-level Stage 2 architectural contract.
