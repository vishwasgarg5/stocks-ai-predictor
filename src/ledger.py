...
def latest_prediction_date(bound=None):
    """Return exact prediction date when a bound is supplied; otherwise latest available date."""
...
def morning_report_sent(prediction_date):
    ...
    if not path.exists(): return False
    try:
        data=json.loads(path.read_text()); return data.get("ReportSent") is True and data.get("DeliveryVersion")=="v2"
    except Exception: return False
...
def save_predictions(df,prediction_date,metadata=None):
    ...
    path=prediction_path(prediction_date); enriched=_baseline_columns(df,metadata)
    if path.exists():
        try:
...
def save_evaluation(df,market_date):
    ...
    for prediction_date,indexes in x.groupby(x["PredictionDate"].astype(str)).groups.items():
        ...
        for idx in indexes:
            ...
            if not pid: raise ValueError(f"No canonical Prediction_ID for {x.at[idx,'Symbol']} on {prediction_date}")
    ...
    if path.exists():
        try:
            if not pd.read_csv(path).empty: return path
...
def rebuild_stock_reliability():
    ...
    for symbol,group in data.groupby("Symbol"):
        apes=[pd.to_numeric(group[f"APE_{t}"],errors="coerce").abs().mean() for t in ("Open","High","Low","Close") if f"APE_{t}" in group.columns]
        close=float(pd.to_numeric(group["APE_Close"],errors="coerce").abs().mean()) if "APE_Close" in group else 3.0
        direction=float(pd.to_numeric(group["DirectionCorrect"],errors="coerce").mean()*100) if "DirectionCorrect" in group else 50.0
...
def save_jump_predictions(df,prediction_date):
    path=jump_path(prediction_date); path.parent.mkdir(parents=True,exist_ok=True); df.to_csv(path,index=False); return path

def save_intraday_predictions(df,prediction_date):
    path=intraday_path(prediction_date); path.parent.mkdir(parents=True,exist_ok=True); df.to_csv(path,index=False); return path

def load_jump_predictions(prediction_date):
    path=jump_path(prediction_date); return pd.read_csv(path) if path.exists() else pd.DataFrame()
def load_intraday_predictions(prediction_date):
    path=intraday_path(prediction_date); return pd.read_csv(path) if path.exists() else pd.DataFrame()
