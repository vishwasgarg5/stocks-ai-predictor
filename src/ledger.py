...
def prediction_exists(prediction_date):
    ...
    try:df=pd.read_csv(path);return len(df)>=1 and "Symbol" in df.columns
...
def save_predictions(df,prediction_date,metadata=None):
    ...
    # Prediction records are immutable for a trading date. Retraining/re-runs must never alter
    # the forecast that the evening evaluator will later score.
    if path.exists():return path
    enriched=_baseline_columns(df,metadata);tmp=path.with_suffix(".tmp");enriched.to_csv(tmp,index=False);tmp.replace(path)
    metadata["PredictionLedgerVersion"]="v3";metadata["PredictionLedgerKey"]="Prediction_ID";metadata["Baseline"]="Previous_Close";metadata["BaselineCostBps"]=float(TRANSACTION_COST_BPS)+float(SLIPPAGE_BPS);metadata["PredictionIDs"]=enriched["Prediction_ID"].astype(str).tolist() if "Prediction_ID" in enriched else []
...
def rebuild_stock_reliability():
    ...
    for symbol,group in data.groupby("Symbol"):
        ...
        close_ape=float(pd.to_numeric(group.get("APE_Close",pd.Series(dtype=float)),errors="coerce").abs().mean()) if "APE_Close" in group else 3.0;direction=float(pd.to_numeric(group.get("DirectionCorrect",pd.Series(dtype=float)),errors="coerce").mean()*100) if "DirectionCorrect" in group else 50.0
