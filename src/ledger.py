...
def save_prediction(ledger, p, rank, score, target_date=None):
    ...
    row.update({"created_at": datetime.now(timezone.utc).isoformat(), "target_date": target_date,
                "symbol": p["symbol"], "rank": rank, "score": score, "base_close": p["base_close"],
                "pred_open": p["pred_open"], "pred_high": p["pred_high"], "pred_low": p["pred_low"], "pred_close": p["pred_close"]})
...
def evaluate_pending(ledger, actuals):
    """Evaluate only against OHLC from exactly the prediction target session."""
    updated = 0
    ledger = ledger.copy()
...
def performance_report(ledger):
    ...
    if done.empty:
        return pd.DataFrame()
    eps = 1e-12
    mape_cols = ["open","high","low","close"]
    mape = {f"{c}_mape": (done[f"{c}_error"].abs() / done[f"actual_{c}"].abs().clip(lower=eps)).mean() for c in mape_cols}
