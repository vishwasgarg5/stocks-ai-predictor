...
def _cached_history(ticker):
    ...
    try:
        if path.exists():
            d=pd.read_csv(path);d["Date"]=pd.to_datetime(d["Date"],errors="coerce") if "Date" in d.columns else pd.to_datetime(d.index,errors="coerce");d=d.set_index("Date") if "Date" in d.columns else d;d=d[~d.index.duplicated(keep="last")].sort_index();
            if len(d)>=150:return d
    except Exception:pass
    try:
        import yfinance as yf
        d=yf.download(ticker,period="3y",auto_adjust=False,progress=False,threads=False)
...
def _portfolio_ai(portfolio,cutoff_date=None,variant="A"):
    ...
    for _,r in portfolio.iterrows():
        ...
        if d.empty or "Close" not in d:print(f"{symbol}: portfolio AI skipped: no history");continue
        try:
            if cutoff_date is None:
                valid0=d.dropna(subset=["Close"]);effective_cutoff=valid0.index[-1] if not valid0.empty else None
            else:effective_cutoff=pd.Timestamp(cutoff_date)
            if effective_cutoff is None:continue
            d=d[d.index<=effective_cutoff].copy();d=d.dropna(subset=["Open","High","Low","Close","Volume"])
            if len(d)<150:print(f"{symbol}: portfolio AI skipped: only {len(d)} rows");continue
            bundle=train_stock_bundle(d,symbol,effective_cutoff,variant,train_horizons=False);pred=predict_stock(d,bundle,effective_cutoff);item={"Symbol":symbol,**pred,"PredictionDate":str(pd.Timestamp(effective_cutoff).date())}
            try:
                hb=train_horizon_models(d,effective_cutoff);h=add_multihorizon_predictions(d,{"horizons":hb},effective_cutoff)
...
def portfolio_snapshot(cutoff_date=None,variant="A"):
    ...
    latest_date=ai.PredictionDate.iloc[0] if not ai.empty and "PredictionDate" in ai.columns else None;prediction_date=str(cutoff_date or latest_date or "-")
    ...
    return df,{"Positions":len(df),"Value":float(value),"PnL":float(pnl),"Return":float(pnl/invested*100) if invested else 0.,"ActionCounts":df["Decision"].value_counts().to_dict(),"Available":True}
