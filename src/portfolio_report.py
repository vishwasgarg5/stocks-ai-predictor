"""Standalone AI Portfolio Manager.

This module is intentionally separate from the core stock-prediction model.
It reads the user's portfolio, attaches the latest prediction horizons and
produces actionable SELL / HOLD / AVG / DO NOT AVG decisions.
"""
...
def _sell_window(row,target_price,current,prediction_date):
    ...
    for h in HORIZONS:
        ...
        horizon_price=float(current)*(1+float(expected)/100)
        if horizon_price>=float(target_price):
...
def _average_plan(row):
    ...
    if pd.isna(target):
        row["Target_Return_Pct"]=None;row["Recommended_Qty"]=0;row["New_Average_Price"]=float(avg);row["Projected_Return_At_AI_Target"]=None;row["Averaging_Action"]="DO NOT AVG";row["Decision"]="HOLD";row["Sell_Window"]="NO AI TARGET";row["Sell_Reason"]="Waiting for validated AI target";return row
    ...
    if price>=profit_target:
        row["Recommended_Qty"]=0;row["New_Average_Price"]=float(avg);row["Projected_Return_At_AI_Target"]=(target/avg-1)*100;row["Averaging_Action"]="DO NOT AVG";row["Decision"]="SELL / PROFIT BOOK";row["Sell_Window"]="NOW";row["Sell_Reason"]=f"{TARGET_PROFIT_PCT:.0f}% portfolio profit target reached";return row
    ...
    if rec>0 and projected>=TARGET_PROFIT_PCT and conf>=MIN_AI_CONFIDENCE and row["Recovery_Gap_Pct"]>=5:
        row["Averaging_Action"]="AVERAGE";row["Decision"]="AVG";row["Sell_Reason"]="AI recovery supports reduced average and profit target"
    elif row["Recovery_Gap_Pct"]<=0:
        row["Averaging_Action"]="DO NOT AVG";row["Decision"]="HOLD";row["Sell_Reason"]="Position is not below average cost"
    else:
        row["Averaging_Action"]="DO NOT AVG";row["Decision"]="HOLD / RECOVERY";row["Sell_Reason"]="Recovery target not strong enough or confidence is low"
    return row
...
def portfolio_snapshot():
    ...
    df["Current_Value"]=df["Quantity"]*df["Current_Price"].fillna(0);mask=df["Reported_PnL"].notna()&df["AveragePriceSource"].str.startswith("ESTIMATED");df["PnL"]=df["Current_Value"]-df["Invested_Value"];df.loc[mask,"PnL"]=df.loc[mask,"Reported_PnL"]
