...
def calculate_jump_score(current_price,predicted_close,predicted_high,confidence,technical):
    ...
    target_score=np.clip(high_return/JUMP_THRESHOLD*100,0,100); close_score=np.clip(close_return/JUMP_THRESHOLD*100,0,100)
    return float(np.clip(0.30*target_score+0.20*close_score+0.25*confidence+0.25*technical,0,100))
...
def generate_jump_watchlist(data_map,cutoff_date,variant="A"):
    ...
    for symbol,df in data_map.items():
        try:
            ...
            current=float(result.get("Current_Price",df["Close"].iloc[-1])); predictions={t:float(result[f"Pred_{t}"]) for t in ["Open","High","Low","Close","Volume"]}
            ...
            if probability<MIN_JUMP_PROBABILITY or true_7d_return<MIN_JUMP_7D_UPSIDE: continue
