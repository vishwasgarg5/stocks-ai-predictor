...
from .config import MULTI_HORIZONS, HISTORY_PERIOD
...
def train_horizon_models(df,cutoff_date):
    # Candidate scans may carry only 3 months. Expand only the stock actually
    # entering the multi-horizon model so the full NSE scan stays bounded.
    x=build_features(df);x=x[x.index<=pd.Timestamp(cutoff_date)].copy();features=get_feature_columns()
    minimum_required=max(180,80+max(HORIZONS)*2)
    if len(x.dropna(subset=[c for c in features if c in x.columns]))<minimum_required:
        try:
            symbol=getattr(df,"name",None)
            if symbol:
                expanded=download_symbol(str(symbol),period=HISTORY_PERIOD)
                if expanded is not None and not expanded.empty:
                    expanded=expanded[expanded.index<=pd.Timestamp(cutoff_date)];x=build_features(expanded);x=x[x.index<=pd.Timestamp(cutoff_date)].copy()
        except Exception as exc:print(f"Multi-horizon long-history expansion unavailable: {exc}")
...
def predict_horizons(df,bundle,cutoff_date):
    ...
    for h in HORIZONS:
        ...
        close=np.array([m.predict(latest)[0] for m in info["close"]["models"]]);ret=np.array([m.predict(latest)[0] for m in info["return"]["models"]]);cw=np.asarray(info["close"]["weights"]);rw=np.asarray(info["return"]["weights"]);close_pred=float(close@cw);return_pred=float(ret@rw)
        rows.append({"HorizonDays":h,"Status":"VALID","Pred_Close":close_pred,"Expected_Return":return_pred,"CloseDerivedReturn":(close_pred/current-1)*100,"ValidationMAPE":info["close"]["validation_mape"],"ReturnValidationMAPE":info["return"]["validation_mape"],"Samples":info["close"]["samples"]})
