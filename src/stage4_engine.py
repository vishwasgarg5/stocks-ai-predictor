...
def add_stage4_context(candidates,data_map,regime):
    ...
    df["PriceBucketLabel"]=df["Current_Price"].map(price_bucket);df["PriceBucket"]=df["Current_Price"].map(price_bucket_code)
    returns={s:_stock_return(data_map.get(s)) for s in df["Symbol"]};df["SectorReturn20D"]=df["Symbol"].map(returns);sector_stats=df.groupby("Sector",dropna=False)["SectorReturn20D"].agg(["median","count"]);overall_median=float(df["SectorReturn20D"].median()) if df["SectorReturn20D"].notna().any() else 0.0;strength=[]
