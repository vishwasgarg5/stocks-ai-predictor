...
def _load_sector_cache():
    ...
    try:
        df = pd.read_csv(path)
        return {str(r["Symbol"]): str(r["Sector"]) for _, r in df.iterrows() if str(r.get("Symbol", "")) and str(r.get("Sector", "UNKNOWN"))}
    except Exception: return {}


def _save_sector_cache(cache):
    path = Path(STAGE4_SECTOR_MAP_FILE); path.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(sorted(cache.items()), columns=["Symbol", "Sector"]).to_csv(path, index=False)
...
def add_stage4_context(candidates, data_map, regime):
    ...
    df = candidates.copy(); sector_map = enrich_sectors(df["Symbol"].tolist())
    df["Sector"] = df["Symbol"].map(sector_map).fillna("UNKNOWN")
    buckets = df["Current_Price"].apply(price_bucket)
    df["PriceBucket"] = buckets.map(lambda x: x[0]); df["PriceBucketLabel"] = buckets.map(lambda x: x[1])
    ...
    for _, row in df.iterrows():
        ...
        if sector in sector_stats.index and pd.notna(ret):
            median_ret = float(sector_stats.loc[sector, "median"]); peer_count = int(sector_stats.loc[sector, "count"])
            momentum = 50.0 + (median_ret - overall_median) * 8.0; breadth_bonus = min(max(peer_count - 1, 0), 5) * 1.5; score = momentum + breadth_bonus
        else: score = 50.0
        strength.append(clamp(score))
    df["SectorStrength"] = strength
    regime_adjustment = {"BULL": 3, "BEAR": -3, "HIGH VOL": -5}.get(regime, 0)
    df["SectorScore"] = (df["SectorStrength"].clip(0, 100) + regime_adjustment).clip(0, 100)
    return df
