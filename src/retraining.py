...
def load_model_state():
    default={"active_variant":"A","previous_variant":None,"last_champion_error":None,"last_challenger_error":None,"last_improvement":None,"last_decision":"INITIAL","baseline_live_mape":None,"promotion_p_value":None,"models_trained":False}
...
def _paired_p_value(champion,challenger,iterations=PROMOTION_BOOTSTRAP_ITERATIONS):
    """Bootstrap a paired stock-level improvement with a null centered at zero.

    Inputs must be paired in the same symbol order. The null distribution is
    generated by independently flipping the sign of each observed paired
    difference; this preserves the paired structure and tests whether the
    observed mean improvement is greater than zero.
    """
    a=np.asarray(champion,dtype=float);b=np.asarray(challenger,dtype=float)
    n=min(len(a),len(b))
    ...
    n=len(d)
    if n<MIN_PROMOTION_SAMPLES:return None
    observed=float(np.mean(d));rng=np.random.default_rng(42)
    signs=rng.choice(np.array([-1.0,1.0]),size=(int(iterations),n),replace=True)
    null_means=(signs*d).mean(axis=1)
    return float((np.sum(null_means>=observed)+1)/(len(null_means)+1)) if observed>0 else 1.0
...
def compare_variants(data_map,symbols,cutoff_date):
    ...
    if not champion_errors:return {"ModelsTrained":False,"ChallengerPromoted":False,"Retrained":False,"Decision":"NO DATA","Champion":active,"Challenger":challenger}
    ce=float(np.mean(champion_errors));xe=float(np.mean(challenger_errors));improvement=(ce-xe)/max(ce,1e-8);live=_live_mape();p=_paired_p_value(champion_errors,challenger_errors)
    ...
    state.update({"active_variant":new_active,"last_champion_error":ce,"last_challenger_error":xe,"last_improvement":improvement*100,"last_decision":decision,"processed_stocks":processed,"cutoff_date":str(pd.Timestamp(cutoff_date).date()),"last_live_mape":live,"promotion_p_value":p,"models_trained":True})
    save_model_state(state);rebuild_stock_reliability()
    return {"ModelsTrained":True,"ChallengerPromoted":switched,"Retrained":switched,"Decision":decision,"Champion":active,"Challenger":challenger,"NewChampion":new_active,"ChampionError":ce,"ChallengerError":xe,"Improvement":improvement*100,"StocksProcessed":len(processed),"LiveMAPE":live,"PromotionPValue":p,"PromotionMinimumSamples":MIN_PROMOTION_SAMPLES}
