...
# The morning runner performs the authoritative full-universe 3mo scan itself.
# The gate must therefore be a bounded live smoke test, not a second full scan.
# Re-downloading thousands of NSE symbols here was both redundant and prone to
# Yahoo throttling, causing the workflow to spend ~50 minutes in the gate and
# never reach the actual prediction pipeline.
MORNING_GATE_SAMPLE = 100
MORNING_GATE_MIN_VALID = 50
...
def _bounded_sample(symbols, limit=MORNING_GATE_SAMPLE):
    """Pick a deterministic spread across the live NSE universe."""
    ...
    return list(dict.fromkeys(unique[i] for i in positions))
...
def morning_gate():
    ...
    if not universe:
        raise RuntimeError("LIVE_DATA_QUALITY_FAILED MORNING: empty NSE universe")

    # Do not duplicate the authoritative full-universe download performed by
    # morning_runner. A deterministic live sample catches provider outages,
    # malformed data, and stale-session problems without blocking production
    # for an hour before the real pipeline even starts.
    sample = _bounded_sample(universe)
    raw = download_many(sample, "3mo", workers=8)
    if len(raw) < min(MORNING_GATE_MIN_VALID, len(sample)):
        raise RuntimeError(
            f"LIVE_DATA_QUALITY_FAILED MORNING sample_coverage={len(raw)}/{len(sample)}"
        )

    fallback = get_completed_session_date("morning")
    cutoff = get_data_cutoff_date(raw, None, fallback=fallback)
    if cutoff is None:
        raise RuntimeError("LIVE_DATA_QUALITY_FAILED MORNING: no completed market cutoff")

    raw = _clip_to_cutoff(raw, cutoff)
    passed, failed = validate_universe(raw, cutoff_date=cutoff, min_rows=30)
    if len(passed) < min(MORNING_GATE_MIN_VALID, len(sample)):
        raise RuntimeError(
            f"LIVE_DATA_QUALITY_FAILED MORNING valid_sample={len(passed)}/{len(sample)} "
            f"failed={len(failed)} sample={_sample(failed)}"
        )

    return {
        "Status": "PASS" if not failed else "PASS_WITH_SAMPLE_WARNINGS",
        "Cutoff": str(cutoff),
        "Sampled": len(sample),
...
def evening_gate():
    ...
    if prediction_date is None:
        raise RuntimeError(
            f"LIVE_DATA_QUALITY_FAILED EVENING: no exact prediction for {market_date}"
        )
    ...
    raw = download_many(symbols, "3mo", workers=5)
    ...
    if missing:
        raise RuntimeError(
            f"LIVE_DATA_QUALITY_FAILED EVENING missing_symbols={','.join(missing[:10])}"
        )
    return {
        "Status": "PASS",
