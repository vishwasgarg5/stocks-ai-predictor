...
def build_features(df):
    ...
    x["SMA10"]=close.rolling(10).mean(); x["SMA20"]=close.rolling(20).mean(); x["SMA50"]=close.rolling(50).mean(); x["EMA9"]=close.ewm(span=9,adjust=False).mean(); x["EMA20"]=close.ewm(span=20,adjust=False).mean(); x["RSI14"]=rsi(close)
    x["MACD"],x["MACD_Signal"],x["MACD_Hist"]=macd(close)
    bb_mid=close.rolling(20).mean(); bb_std=close.rolling(20).std(); x["BB_Mid"]=bb_mid; x["BB_Upper"]=bb_mid+2*bb_std; x["BB_Lower"]=bb_mid-2*bb_std; x["BB_Position"]=(close-x["BB_Lower"])/(x["BB_Upper"]-x["BB_Lower"]).replace(0,np.nan)
    x["ATR14"]=atr(x); x["ATR_Pct"]=x["ATR14"]/close.replace(0,np.nan); x["ADX14"]=adx(x)
    for n in [1,3,5,10,20]: x[f"Return{n}"]=close.pct_change(n)
    x["Volatility10"]=x["Return1"].rolling(10).std(); x["Volatility20"]=x["Return1"].rolling(20).std(); volume_ma20=volume.rolling(20).mean(); x["Volume_Ratio"]=volume/volume_ma20.replace(0,np.nan)
    direction=np.sign(close.diff()).fillna(0); x["OBV"]=(direction*volume).cumsum(); x["OBV_Change"]=x["OBV"].pct_change(10)
