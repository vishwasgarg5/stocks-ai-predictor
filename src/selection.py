"""Stage 28 precision selection with coverage-first prediction selection."""
import pandas as pd
...
def _bucket_cap(df,max_per_bucket):
    ...
    for bucket,group in df.groupby("PriceBucket",sort=False):
        g=group.sort_values(["TradeConfidence","Score","Confidence","Direction_Confidence","Symbol"],ascending=[False,False,False,False,True]);limit=len(g) if max_per_bucket is None or max_per_bucket<=0 else min(len(g),int(max_per_bucket));pieces.append(g.head(limit))
...
def score_candidates(candidates,regime="SIDEWAYS"):
    ...
    return df.sort_values(["TradeConfidence","Score","Confidence","Direction_Confidence","SectorScore","Symbol"],ascending=[False,False,False,False,False,True]).reset_index(drop=True)

def select_prediction_set(candidates,top_n=TOP_N,max_per_bucket=MAX_PER_PRICE_BUCKET,regime="SIDEWAYS"):
    """Coverage-first prediction set: valid forecasts fill buckets before trade filters."""
    ...
    n=min(int(top_n),len(pool)) if top_n is not None and int(top_n)>0 else len(pool)
    # Prefer low uncertainty, but never let uncertainty/BUY eligibility collapse coverage.
    preferred=_apply_uncertainty_cap(pool)
    ranked=_bucket_cap(preferred,max_per_bucket)
    if len(ranked)<n:ranked=_bucket_cap(pool,max_per_bucket)
    # Seed one best valid stock from every available bucket, then fill by score.
    chosen=[]
    for bucket in sorted(ranked["PriceBucket"].astype(str).unique()):
        g=ranked[ranked["PriceBucket"].astype(str)==bucket]
        if not g.empty:chosen.append(g.iloc[0])
    chosen=pd.DataFrame(chosen).drop_duplicates("Symbol") if chosen else ranked.iloc[0:0]
    if len(chosen)<n:
        remaining=ranked[~ranked["Symbol"].isin(chosen["Symbol"])].sort_values(["TradeConfidence","Score","Confidence","Direction_Confidence"],ascending=False)
        chosen=pd.concat([chosen,remaining.head(n-len(chosen))],ignore_index=True)
    return chosen.drop_duplicates("Symbol").head(n).reset_index(drop=True)
