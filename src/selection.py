"""Stage 10.5 precision selection: qualified stocks with bucket-aware allocation."""
import pandas as pd
...
def select_top_stocks(candidates,top_n=TOP_N,regime="SIDEWAYS",min_score=65.0,min_confidence=60.0,min_trade_confidence=60.0,max_per_bucket=MAX_PER_PRICE_BUCKET,bucket_only=False):
    """Select up to max_per_bucket per price bucket. For a global top-N, seed each qualified bucket first, then fill remaining slots by score."""
    ...
    n=int(top_n)
    # One best qualified stock per bucket prevents a global top-N from starving low-priced buckets.
    seeded=[g.iloc[0] for _,g in groups if not g.empty]
    chosen=pd.DataFrame(seeded).drop_duplicates(subset=["Symbol"]) if seeded else qualified.iloc[0:0]
    if len(chosen)<n:
        remaining=qualified[~qualified["Symbol"].isin(chosen["Symbol"])]
        remaining=remaining.sort_values(["TradeConfidence","Score","Confidence","Direction_Confidence"],ascending=False)
        chosen=pd.concat([chosen,remaining.head(n-len(chosen))],ignore_index=True)
    elif len(chosen)>n:
        chosen=chosen.sort_values(["TradeConfidence","Score"],ascending=False).head(n)
