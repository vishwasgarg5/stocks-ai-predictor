...
def select_prediction_set(candidates,top_n=TOP_N,max_per_bucket=MAX_PER_PRICE_BUCKET,regime="SIDEWAYS"):
    """Select predictions independently of BUY/TRADE eligibility."""
    scored=score_candidates(candidates,regime)
    if scored.empty:return scored
    pool=scored.copy();required=["Pred_Open","Pred_High","Pred_Low","Pred_Close","Current_Price"]
    ...
    n=int(top_n) if top_n is not None and int(top_n)>0 else len(pool)
    ranked=_bucket_cap(pool,max_per_bucket)
    preferred=_bucket_cap(_apply_uncertainty_cap(pool),max_per_bucket)
    # Uncertainty is a preference, not a reason to publish fewer predictions.
    source=preferred if len(preferred)>=min(n,len(ranked)) else ranked
    seeded=[]
    for _,group in source.groupby("PriceBucket",sort=False):
        if not group.empty:seeded.append(group.iloc[0])
    chosen=pd.DataFrame(seeded).drop_duplicates("Symbol") if seeded else source.iloc[0:0]
    if len(chosen)<n:
        remaining=source[~source["Symbol"].isin(chosen["Symbol"])].head(n-len(chosen));chosen=pd.concat([chosen,remaining],ignore_index=True)
