"""Stage 10.4 weekly report: accuracy, reliability, calibration, decisions and model health."""
import math
import pandas as pd
...
def _load_recent_evaluations(days=5):
    ...
    x=pd.concat(frames,ignore_index=True);x["MarketDate"]=pd.to_datetime(x.get("MarketDate"),errors="coerce");x=x.dropna(subset=["MarketDate"]).sort_values("MarketDate");dates=x["MarketDate"].dt.date.drop_duplicates().tolist()[-days:];return x[x["MarketDate"].dt.date.isin(dates)]

def _bucket_summary(evals):
    if evals.empty or "PriceBucket" not in evals or "APE_Close" not in evals:return []
    rows=[]
    for b,g in evals.groupby("PriceBucket"):rows.append((str(b),len(g),float(g["APE_Close"].abs().mean()),float(g["DirectionCorrect"].mean()*100) if "DirectionCorrect" in g else 0))
...
def _horizon_summary():
    ...
    if x.empty:return [],[]
    x["EvaluatedDate"]=pd.to_datetime(x.get("EvaluatedDate"),errors="coerce");cutoff=x["EvaluatedDate"].max()-pd.Timedelta(days=14);x=x[x["EvaluatedDate"]>=cutoff];out=[]
    for h,g in x.groupby("HorizonDays"):out.append((int(h),len(g),float(g["APE"].abs().mean()),float((1-g["APE"].abs()/100).clip(0,1).mean()*100),float(g["DirectionCorrect"].mean()*100)))
    bh=[]
    if "PriceBucket" in x:
...
def _wilson(successes,total,z=1.96):
    ...
    p=successes/total;den=1+z*z/total;centre=(p+z*z/(2*total))/den;half=z*math.sqrt((p*(1-p)+z*z/(4*total))/total)/den;return max(0,(centre-half)*100),min(100,(centre+half)*100)

def _model_health(recent,decision):
    if recent.empty:return "NO VALIDATION"
    mape=float(recent["CloseMAPE"].mean());direction=float(recent["DirectionAccuracy"].mean());score=0.35*max(0,100-min(mape*10,100))+0.35*direction+0.15*min(100,len(recent)*20)+0.15*(decision.get("WinRate") or 50);return f"{score:.0f}/100"
...
def run():
    ...
    required=["MarketDate","Samples","OpenMAPE","HighMAPE","LowMAPE","CloseMAPE","OverallMAPE","DirectionAccuracy"];missing=[c for c in required if c not in df.columns]
    ...
    if bucket_horizons:
        best=bucket_horizons[0];worst=bucket_horizons[-1];lines += ["","Bucket × Horizon",f"Best: {best[0]} / {best[1]}D | {best[2]} samples | {best[3]:.2f}% MAPE",f"Worst: {worst[0]} / {worst[1]}D | {worst[2]} samples | {worst[3]:.2f}% MAPE"]
