"""Weekly Stage 10.1 performance report with bucket, horizon, confidence and learning diagnostics."""
import pandas as pd
...
def _confidence_summary(evals):
    ...
    x=evals.copy();x["PredictionConfidence"]=pd.to_numeric(x["PredictionConfidence"],errors="coerce");x=x.dropna(subset=["PredictionConfidence"])
    if x.empty:return []
    out=[]
    for name,lo,hi in [("High",80,101),("Medium",60,80),("Low",-1,60)]:
        g=x[(x["PredictionConfidence"]>=lo)&(x["PredictionConfidence"]<hi)]
        if not g.empty:out.append((name,len(g),float(g["APE_Close"].abs().mean()),float(g["DirectionCorrect"].mean()*100) if "DirectionCorrect" in g else 0.0))
    return out


def _error_distribution(evals):
    if evals.empty or "APE_Close" not in evals.columns:return []
    a=evals["APE_Close"].abs().dropna()
    if a.empty:return []
    total=len(a)
    return [(label,int((a<=limit).sum()/total*100)) for label,limit in [("≤1%",1),("≤2%",2),("≤3%",3),("≤5%",5)]]
...
def _horizon_summary():
    ...
    if "EvaluatedDate" in x.columns:
        x["EvaluatedDate"]=pd.to_datetime(x["EvaluatedDate"],errors="coerce");cutoff=x["EvaluatedDate"].max()-pd.Timedelta(days=14);x=x[x["EvaluatedDate"]>=cutoff]
    if x.empty:return []
    out=[]
    for h,g in x.groupby("HorizonDays"):
        out.append((int(h),len(g),float(g["APE"].abs().mean()),float((1-g["APE"].abs()/100).clip(0,1).mean()*100),float(g["DirectionCorrect"].mean()*100) if "DirectionCorrect" in g else 0.0))
    return sorted(out)
...
def run():
    ...
    evals=_load_recent_evaluations(5);buckets=_bucket_summary(evals);confidence=_confidence_summary(evals);errors=_error_distribution(evals);horizons=_horizon_summary();state=read_json(MODEL_STATE_FILE,{}) or {}
    ...
    if errors:
        lines += ["","Close Error Distribution"]+[f"Within {label}: {pct}%" for label,pct in errors]
    if buckets:
        lines += ["","Bucket Performance","Bucket | Samples | Close MAPE | Direction"]+[f"{b} | {n} | {m:.2f}% | {d:.1f}%" for b,n,m,d in buckets]+[f"Best bucket: {buckets[0][0]}",f"Worst bucket: {buckets[-1][0]}"]
    if horizons:
        lines += ["","Horizon Performance","Horizon | Samples | MAPE | Accuracy | Direction"]+[f"{h}D | {n} | {m:.2f}% | {a:.1f}% | {d:.1f}%" for h,n,m,a,d in horizons]
    if confidence:
        lines += ["","Confidence Calibration","Band | Samples | Close MAPE | Direction"]+[f"{b} | {n} | {m:.2f}% | {d:.1f}%" for b,n,m,d in confidence]
    if len(recent)>=2:
        prev=recent.iloc[-2];cur=recent.iloc[-1];lines += ["","Learning Trend",f"Close MAPE: {float(prev['CloseMAPE']):.3f}% → {float(cur['CloseMAPE']):.3f}%",f"Direction: {float(prev['DirectionAccuracy']):.1f}% → {float(cur['DirectionAccuracy']):.1f}%"]
    active=state.get("active_variant","-");decision=state.get("last_decision","-");improvement=state.get("last_improvement")
    improvement_text="-" if improvement is None else f"{float(improvement):+.2f}%"
    lines += ["","Model Learning",f"Champion: {active}",f"Last Decision: {decision}",f"Last Validation Improvement: {improvement_text}",f"Learning State: {'available' if FINAL_LEARNING_STATE_FILE.exists() else 'missing'}","Results are stored in GitHub state and used for future stock/model reliability."]
    send_telegram("\n".join(lines))
