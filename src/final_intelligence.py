...
def prediction_interval(row,z=1.645):
    base=_num(row.get("Pred_Close",row.get("Current_Price",0)),0); u=max(_num(row.get("PredictionUncertaintyPct",5),5),0)/100; half=base*u*z
...
def regime_risk_adjustment(regime,score):
    return float(np.clip(_num(score)*{"BULL":1.05,"SIDEWAYS":1,"BEAR":.88,"HIGH VOL":.78}.get(str(regime).upper(),1),0,100))
...
def final_action(row):
    s=_num(row.get("FinalDecisionScore",row.get("DecisionScore",50))); direction=str(row.get("Direction","NEUTRAL")).upper()
