...
def apply_final_intelligence(candidates,regime="SIDEWAYS",breadth=50,news=50):
    ...
    hp=pd.DataFrame(probs,index=out.index);out[hp.columns]=hp;out["TargetProbabilityModel"]=modes;out["TargetProbabilitySamples"]=ps;out["BreadthScore"]=_num(breadth);out["NewsEventScore"]=_num(news);out["RegimeAdjustedScore"]=out.apply(lambda r:float(np.clip(_num(r.get("RiskAdjustedScore",r.get("Score",50)))*{"BULL":1.05,"SIDEWAYS":1,"BEAR":.88,"HIGH VOL":.78}.get(str(regime).upper(),1),0,100)),axis=1);out["NetExpectedReturn"]=pd.to_numeric(out.get("Expected_Return",0),errors="coerce").fillna(0).apply(net_expected_return);out["BenchmarkEdgePct"]=pd.to_numeric(out.get("Expected_Return",0),errors="coerce").fillna(0)-pd.to_numeric(out.get("BenchmarkExpectedReturn",0),errors="coerce").fillna(0);sr=[];ss=[];hr=[];hs=[]
