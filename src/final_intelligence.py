"""Stage 10.5: calibrated probability, reliability, risk and adaptive decisions."""
...
def benchmark_expected_return(current_price,history=None):
    ...
    c=pd.to_numeric(history.get("Close"),errors="coerce").dropna();return float((c.iloc[-1]/c.iloc[-6]-1)*100) if len(c)>=6 else float((c.iloc[-1]/c.iloc[-2]-1)*100)
...
def _trained_probs(row):
    ...
    try:
        ...
        X=d[cols].apply(pd.to_numeric,errors="coerce").fillna(0);xr=pd.DataFrame([{c:_num(row.get(c,0),0) for c in cols}],columns=cols);from sklearn.linear_model import LogisticRegression;out={}
...
def decision_score(row,regime="SIDEWAYS",breadth=50,news=50):
    e=_num(row.get("Expected_Return",0),0);c=_num(row.get("CalibratedConfidence",row.get("Confidence",50)));risk=_num(row.get("RegimeAdjustedScore",row.get("RiskAdjustedScore",row.get("Score",50))));t=_num(row.get("TechnicalScore",50));mh=_num(row.get("MultiHorizonExpectedReturn",0),0);hit=_num(row.get("TargetHitProb_3_0Pct",50),50);edge=_num(row.get("BenchmarkEdgePct",0),0);rr=_num(row.get("RiskAdjustedReturn",0),0);sr=_num(row.get("StockReliability",50),50);dq=_num(row.get("DataQualityScore",100),100);return float(np.clip(.19*risk+.14*c+.11*t+.10*np.clip(50+net_expected_return(e)*5,0,100)+.10*np.clip(50+mh*4,0,100)+.09*hit+.08*_num(breadth)+.05*_num(news)+.05*np.clip(50+edge*5,0,100)+.04*np.clip(50+rr*10,0,100)+.03*sr+.02*dq,0,100))
def final_action(row,regime="SIDEWAYS"):
    ...
    if u>15 or c<45 or dq<60 or sr<30 or hr<30 or s<55 or n<min_net or down>=75:return "NO TRADE"
    if s>=buy and d=="UP" and edge>=-BENCHMARK_TOLERANCE_PCT and hit>=50 and down<75:return "BUY"
    if s>=watch and d=="UP" and hit>=45 and down<80:return "WATCH"
...
def apply_final_intelligence(candidates,regime="SIDEWAYS",breadth=50,news=50):
    ...
    out=candidates.copy();out["Regime"]=regime;out["CalibratedConfidence"]=out.apply(lambda r:calibrate_confidence(r.get("Confidence",50),r.get("ReliabilityScore",50),r.get("ReliabilitySamples",0)),axis=1);out["RiskAdjustedReturn"]=out.apply(lambda r:volatility_adjusted_return(r.get("Expected_Return",0),r.get("VolatilityPct",r.get("PredictionUncertaintyPct",5))),axis=1);probs=[];modes=[];ps=[]
    for _,r in out.iterrows():p,m,n=_trained_probs(r);probs.append(p);modes.append(m);ps.append(n)
    hp=pd.DataFrame(probs,index=out.index);out[hp.columns]=hp;out["TargetProbabilityModel"]=modes;out["TargetProbabilitySamples"]=ps;out["BreadthScore"]=_num(breadth);out["NewsEventScore"]=_num(news);out["RegimeAdjustedScore"]=out.apply(lambda r:float(np.clip(_num(r.get("RiskAdjustedScore",r.get("Score",50)))*{"BULL":1.05,"SIDEWAYS":1,"BEAR":.88,"HIGH VOL":.78}.get(str(regime).upper(),1),0,100)),axis=1)
    expected=pd.to_numeric(out["Expected_Return"] if "Expected_Return" in out.columns else pd.Series(0,index=out.index),errors="coerce").fillna(0)
    benchmark=pd.to_numeric(out["BenchmarkExpectedReturn"] if "BenchmarkExpectedReturn" in out.columns else pd.Series(0,index=out.index),errors="coerce").fillna(0)
    out["NetExpectedReturn"]=expected.apply(net_expected_return);out["BenchmarkEdgePct"]=expected-benchmark;sr=[];ss=[];hr=[];hs=[]
    for _,r in out.iterrows():a,n=_stock_reliability(r.get("Symbol",""));h,hn=_horizon_reliability(r.get("Symbol",""),int(r.get("PrimaryHorizon",1) or 1));sr.append(a);ss.append(n);hr.append(h);hs.append(hn)
    ...
    uncertainty=pd.to_numeric(out["PredictionUncertaintyPct"] if "PredictionUncertaintyPct" in out.columns else pd.Series(0,index=out.index),errors="coerce").fillna(0)
    out["UncertaintyFlag"]=uncertainty>15;out["FinalRisk"]=np.select([out["FinalDecisionScore"]>=75,out["FinalDecisionScore"]>=55],["LOW","MEDIUM"],default="HIGH");return out.sort_values(["FinalDecisionScore","Score"],ascending=False).reset_index(drop=True)
...
def update_learning_state(path:Path,observations:dict):
    ...
    state.setdefault("version","final-v9");state.setdefault("observations",[]);state["observations"].append(observations);state["observations"]=state["observations"][-250:];state["last_update"]=observations.get("date");path.write_text(json.dumps(state,indent=2,default=str));return state
def final_stage_manifest():return {"Stage10.5":"Adaptive probability, reliability, risk, cost, portfolio timing and exit learning","Validation":"Historical classifiers use only closed/evaluated outcomes; primary model remains walk-forward","Abstention":"NO TRADE for materially uncertain, weak-data or high-downside cases","Learning":"Outcomes, horizon decay, drift, quarantine and rollback"}
