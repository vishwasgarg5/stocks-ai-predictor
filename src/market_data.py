...
def download_nse_equity_list():
    try:
        response = requests.get(NSE_EQUITY_URL, timeout=20, headers={"User-Agent":"Mozilla/5.0","Accept":"text/csv,*/*"})
        if response.status_code != 200: return []
        from io import StringIO
        df = pd.read_csv(StringIO(response.text))
        column = next((c for c in ["SYMBOL","Symbol","symbol"] if c in df.columns), None)
        if column is None: return []
        return list(dict.fromkeys(normalize_symbol(x) for x in df[column].dropna()))
    except Exception as exc:
        print(f"NSE universe download failed: {exc}")
        return []

def load_universe():
    """Prefer the live broad NSE equity list; repository lists are fallbacks."""
    symbols = download_nse_equity_list()
    if symbols:
        print(f"Using broad NSE universe: {len(symbols)} stocks")
...
def download_symbol(symbol, period=HISTORY_PERIOD):
    try:
        df = yf.download(f"{normalize_symbol(symbol)}.NS", period=period, interval="1d", auto_adjust=False, progress=False, threads=False)
        df = clean_ohlcv(df)
        return df if len(df) >= 30 else None
    except Exception as exc:
        print(f"{symbol}: data download failed: {exc}"); return None
...
def download_many(symbols, period=HISTORY_PERIOD, workers=8):
    ...
    print(f"Downloaded usable data for {len(result)}/{len(symbols)} stocks")
    return result
...
def get_previous_session_date(session_date):
    ...
    if df.empty: return None
    dates=sorted(set(x.date() for x in df.index)); previous=[x for x in dates if x < session_date]
...
def get_row_for_date(df,target_date):
    ...
    for index in df.index:
        if index.date()==target_date:return df.loc[index]
    return None

def get_previous_row(df,target_date):
    if df is None or df.empty:return None
    target_date=pd.Timestamp(target_date).date(); rows=[(i,r) for i,r in df.iterrows() if i.date()<target_date]
