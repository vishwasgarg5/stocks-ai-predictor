"""Stage 4.2 morning pipeline: quality top stocks + jump + intraday."""
import pandas as pd
...
def _bucket_candidates(candidates, max_per_bucket=3):
    """Keep a small strong pool per available price bucket; do not force coverage."""
    if candidates is None or candidates.empty:
        return candidates.iloc[0:0] if candidates is not None else pd.DataFrame()
    pieces = []
    for _, group in candidates.groupby("PriceBucket", sort=False):
        if group.empty:
            continue
        pieces.append(group.sort_values(["Score", "Confidence", "Direction_Confidence"], ascending=False).head(max_per_bucket))
...
def _attach_horizons(candidates, data_map, cutoff_date):
    """Train expensive horizon models only for the small post-bucket candidate pool."""
    frames = []
    for _, row in candidates.iterrows():
        symbol = row["Symbol"]
        try:
            horizons = train_stock_bundle(data_map[symbol], symbol, cutoff_date, row.get("ModelVariant", "A"), train_horizons=True)["horizons"]
            h = add_multihorizon_predictions(data_map[symbol], {"horizons": horizons}, cutoff_date)
            if h.empty:
                row["MultiHorizonExpectedReturn"] = 0.0
            else:
                # Robust mean: reduce the effect of one extreme long-horizon forecast.
                returns = h["Expected_Return"].astype(float).clip(-50, 50)
                row["MultiHorizonExpectedReturn"] = float(returns.median())
                for horizon in [1, 3, 5, 7, 20]:
                    match = h[h["HorizonDays"] == horizon]
                    if not match.empty:
                        row[f"Horizon_{horizon}D"] = float(match.iloc[0]["Expected_Return"])
        except Exception as exc:
            print(f"{symbol}: horizon prediction failed: {exc}")
            row["MultiHorizonExpectedReturn"] = 0.0
        frames.append(row)
    return pd.DataFrame(frames) if frames else candidates.iloc[0:0]
...
def run():
    ...
    # Latest completed equity session; never use today's partial bar.
    ...
    if cutoff_date is None:
        raise RuntimeError("Unable to determine completed market cutoff.")
    ...
    # Stage 2 OHLCV only for all prescreened candidates; horizon models are delayed.
    candidate_rows = []
    ...
    if not candidate_rows:
        raise RuntimeError("Unable to generate predictions.")
    ...
    if candidates.empty:
        raise RuntimeError("No candidates inside configured price buckets.")
    candidates = score_candidates(candidates, regime)

    # Price-bucket diversity is a candidate constraint, not a forced final output.
    bucket_pool = _bucket_candidates(candidates, max_per_bucket=3)
    bucket_pool = bucket_pool.head(15).reset_index(drop=True)
    bucket_pool = _attach_horizons(bucket_pool, data_map, cutoff_date)
    bucket_pool = score_candidates(bucket_pool, regime)

    selected = select_top_stocks(bucket_pool, TOP_N, regime, min_score=65.0, min_confidence=60.0)
