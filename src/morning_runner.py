...
def run():
    ...
    bucket_pool=_bucket_candidates(candidates,3).head(15).reset_index(drop=True)
    bucket_pool=_attach_horizons(bucket_pool,data_map,cutoff_date); bucket_pool=score_candidates(bucket_pool,regime)
    selected=select_top_stocks(bucket_pool,TOP_N,regime,min_score=65.0,min_confidence=60.0); selected["PredictionDate"]=str(prediction_date)
    metadata={"Stage":"Stage 4.2","PredictionDate":str(prediction_date),"DataCutoff":str(cutoff_date),"ModelVariant":variant,"ModelVersion":MODEL_VERSION,"Regime":regime,"PriceBuckets":[">1000","500-999","100-499","50-99","10-49"],"MultiHorizons":[1,3,5,7,20],"SelectedStocks":selected["Symbol"].tolist()}
