...
def _bucket_candidates(candidates,max_per_bucket=2):
    """Keep at most max_per_bucket strongest candidates from every configured bucket."""
...
def _attach_horizons(candidates,data_map,cutoff_date):
    """Train only the horizon models here; OHLC models are not retrained."""
    rows=[]
    for _,row in candidates.iterrows():
        ...
        try:
            ...
            if h.empty:
                ...
                rr=h["Expected_Return"].astype(float).clip(-50,50)
                row["MultiHorizonExpectedReturn"]=float(rr.median())
                for horizon in [1,3,5,7,20]:
                    m=h[h["HorizonDays"]==horizon]
                    if not m.empty:
                        row[f"Horizon_{horizon}D"]=float(m.iloc[0]["Expected_Return"])
...
def run():
    ...
    for symbol,df in data_map.items():
        try:
            scored.append((symbol,technical_score(df[df.index<=pd.Timestamp(cutoff_date)])))
        except Exception:
            pass
    ...
    # Stage 3A prefilter: two strongest candidates in every price bucket.
    bucket_pool=_bucket_candidates(candidates,MAX_PER_PRICE_BUCKET).reset_index(drop=True)
    # Stage 3B: enrich only this small pool to keep the morning run efficient.
    bucket_pool=_attach_horizons(bucket_pool,data_map,cutoff_date)
    bucket_pool=score_candidates(bucket_pool,regime)

    # Final Stage 4.2 selection: up to two qualified stocks per bucket, max ten.
    selected=select_top_stocks(
        ...
        max_per_bucket=MAX_PER_PRICE_BUCKET,
    )
    ...
    metadata={
        ...
        "MaxPerPriceBucket":MAX_PER_PRICE_BUCKET,
        "MaxSelectedStocks":TOP_N,
