...
import pandas as pd
from .config import PRESCREEN_N,HISTORY_PERIOD,JUMP_CANDIDATE_N,MODEL_VERSION,TOP_N,MAX_PER_PRICE_BUCKET,FINAL_LEARNING_STATE_FILE,IPO_METRICS_FILE
...
def run():
    ...
    report=morning_report(prediction_date,cutoff_date,selected,jump_watchlist,intraday,market_snapshot=snapshot,regime=regime,ipo=ipo);send_telegram(report);print(report)
