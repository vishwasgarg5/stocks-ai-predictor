"""Stage 4 prediction API: next-session OHLCV plus Stage 3B horizons."""
import numpy as np
import pandas as pd
...
def train_stock_bundle(df,symbol,cutoff_date,variant="A"):
    ...
    for target in TARGETS:
        bundle=fit_target_ensemble(X,supervised[f"Target_{target}"],variant); target_bundles[target]=bundle; validation_mape.append(bundle["validation_mape"]); validation_error.append(bundle["validation_error"])
...
def _enforce_ohlc_consistency(predictions):
    """Prevent impossible independent-regressor combinations such as High < Close."""
    o=float(predictions["Open"]); h=float(predictions["High"]); l=float(predictions["Low"]); c=float(predictions["Close"])
    predictions["High"]=max(h,o,c); predictions["Low"]=min(l,o,c)
    return predictions
...
def predict_stock(df,bundle,cutoff_date):
    ...
    for target in TARGETS:
        ...
        predictions[target]=max(value,0.0) if target=="Volume" else value; agreements.append(float(model_agreement(component_predictions,final_prediction)[0]))
    ...
    return {"Current_Price":current_close,"Current_Volume":float(latest["Volume"].iloc[0]),"Pred_Open":predictions["Open"],"Pred_High":predictions["High"],"Pred_Low":predictions["Low"],"Pred_Close":predictions["Close"],"Pred_Volume":predictions["Volume"],"Expected_Return":expected_return*100,"Direction":direction_map.get(direction_label,"NEUTRAL"),"Direction_Confidence":direction_probability,"Confidence":confidence,"TechnicalScore":technical_score(df[df.index<=cutoff]),"ValidationMAPE":bundle["validation_mape"],"ValidationError":bundle["validation_error"],"DirectionValidationAccuracy":bundle["direction_validation_accuracy"]}
