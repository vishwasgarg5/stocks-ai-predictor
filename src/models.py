...
def create_regressors(variant="A"):
    if variant=="A":
        ...
        rf_params={"n_estimators":220,"max_depth":12,"min_samples_leaf":2}
        et_params={"n_estimators":220,"max_depth":14,"min_samples_leaf":2}
    else:
        xgb_params={"n_estimators":350,"learning_rate":0.025,"max_depth":4,"min_child_weight":2,"subsample":0.90,"colsample_bytree":0.90}
        rf_params={"n_estimators":300,"max_depth":15,"min_samples_leaf":1}
        et_params={"n_estimators":300,"max_depth":18,"min_samples_leaf":1}
    return {
        "XGB":XGBRegressor(**xgb_params,objective="reg:squarederror",random_state=42,n_jobs=2,verbosity=0),
        "RF":RandomForestRegressor(**rf_params,random_state=42,n_jobs=2),
        "ET":ExtraTreesRegressor(**et_params,random_state=42,n_jobs=2),
    }

def create_direction_model(variant="A"):
    if variant=="A":
        return XGBClassifier(n_estimators=180,learning_rate=0.04,max_depth=4,min_child_weight=2,subsample=0.85,colsample_bytree=0.85,objective="multi:softprob",eval_metric="mlogloss",random_state=42,n_jobs=2,verbosity=0)
...
def fit_target_ensemble(X,y,variant="A"):
    ...
    for name,prediction in validation_predictions.items(): ensemble_validation += weights[name]*prediction
...
def model_agreement(predictions,final_prediction):
    values=np.column_stack(list(predictions.values())); spread=np.std(values,axis=1); denominator=np.maximum(np.abs(final_prediction),1e-8); relative_spread=spread/denominator
    return np.clip(100*(1-np.clip(relative_spread,0,1)),0,100)

def fit_direction_model(X,y,variant="A"):
    split=max(50,min(int(len(X)*(1-VALIDATION_FRACTION)),len(X)-20)); X_train=X.iloc[:split]; X_val=X.iloc[split:]; y_train=y.iloc[:split]; y_val=y.iloc[split:]
    unique_classes=len(np.unique(y_train))
    model=RandomForestClassifier(n_estimators=150,random_state=42,n_jobs=2) if unique_classes<2 else create_direction_model(variant)
    model.fit(X_train,y_train); validation_accuracy=(model.predict(X_val)==y_val).mean()*100
    params={key:value for key,value in model.get_params().items() if key!="n_jobs"}; final_model=model.__class__(**params,n_jobs=2); final_model.fit(X,y)
