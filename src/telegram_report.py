"""Defensive Telegram renderer for Stage 28."""
...
def _table(headers,rows,max_width=14):
 ...
 w=[min(max(len(str(h)),max((len(r[i]) for r in rows),default=0)),max_width) for i,h in enumerate(headers)]
 fit=lambda v,n:v if len(v)<=n else v[:max(1,n-1)]+"…"
 line=lambda r:" | ".join(fit(r[i],w[i]).ljust(w[i]) for i in range(len(w)))
 sep="-"*(sum(w)+3*(len(w)-1)+2)
...
def _report_messages(text):
 return [p for g in [x.strip() for x in text.split(_SECTION) if x.strip()] for p in _split_safe(g,TELEGRAM_MAX_LENGTH)]
def send_telegram(text):
 token,chat_id=os.getenv("TELEGRAM_BOT_TOKEN"),os.getenv("TELEGRAM_CHAT_ID")
 if not token or not chat_id: return False
 msgs=_report_messages(text)
 return bool(msgs) and all(_post_telegram(x,token,chat_id) for x in msgs)
...
def _bucket_sections(s):
 if s is None or s.empty or "PriceBucket" not in s.columns: return ["🎯 *PREDICTION SET BY PRICE BUCKET*","No high-confidence setup today."]
...
def _best_pick_table(s):
 if s is None or s.empty or "PriceBucket" not in s.columns: return ["🏆 *BEST PICK — 1 PER PRICE BUCKET*","No high-confidence setup today."]
 rows=[]
 for b in _ordered_price_buckets(s.PriceBucket):
 ...
 return ["🏆 *BEST PICK — 1 PER PRICE BUCKET*",*_table(["Bucket","Stock","CMP","Exp","Action"],rows)] if rows else ["🏆 *BEST PICK — 1 PER PRICE BUCKET*","No high-confidence setup today."]
...
def _portfolio_horizon_status(x):
 status=str(x.get("Portfolio_Target_Status","") or "")
 if "CONFIRMED" in status.upper(): return "🟢 CONFIRMED"
 return _status_from_values([_num(x.get(f"Horizon_{h}D")) for h in REPORT_HORIZONS])
def _portfolio(p):
...
def _evaluation_sections(e):
 ...
 for _,r in e.iterrows():
  rows=[[k,*[_fmt(r.get(f"{p}{f}")) for f in ("Open","High","Low","Close")]] for k,p in (("Predicted","Pred_"),("Actual","Actual_"))]
  out += [f"💎 *{r.get('Symbol',r.get('Stock','-'))}*",*_table(["Type","Open","High","Low","Close"],rows)]
 return out
...
def morning_report(prediction_date,cutoff_date,selected,jump_watchlist,intraday,**kwargs):
 acc,scan=kwargs.get('accuracy',{}),kwargs.get('scan',{})
