...
def _table(headers,rows):
    if not rows:return ""
    all_rows=[headers]+[[str(x) for x in row] for row in rows];widths=[max(len(row[i]) for row in all_rows) for i in range(len(headers))];sep="  ".join("-"*w for w in widths);body=["  ".join(str(row[i]).ljust(widths[i]) for i in range(len(headers))) for row in all_rows];return "\n".join([body[0],sep]+body[1:])
...
def _accuracy_lines(a):
    return [f"🤖 Accuracy: Previous {_fmt_accuracy(a.get('PreviousAccuracy'))} → Current {_fmt_accuracy(a.get('CurrentAccuracy'))} | Validated {int(a.get('AccuracySamples',a.get('Samples',0)) or 0)} samples",f"📏 Horizons: 1D {_fmt_accuracy(a.get('1D'))} | 3D {_fmt_accuracy(a.get('3D'))} | 5D {_fmt_accuracy(a.get('5D'))} | 7D {_fmt_accuracy(a.get('7D'))} | 20D {_fmt_accuracy(a.get('20D'))} | Direction {_fmt_accuracy(a.get('DirectionAccuracy'))}",f"🧠 Health: {a.get('Health','-')} | Drift {a.get('Drift','-')} | 7D trend {_fmt_accuracy(a.get('Trend7D'))} | 30D trend {_fmt_accuracy(a.get('Trend30D'))}"]
...
def _sector_lines(selected):
    if selected is None or selected.empty or "Sector" not in selected.columns:return []
    x=selected.copy();x["_score"]=pd.to_numeric(x.get("SectorStrength",50),errors="coerce").fillna(50);g=x.groupby("Sector")["_score"].mean().sort_values(ascending=False).head(5)
    return ["🏭 Sector AI: "+" | ".join(f"{k} {v:.0f}" for k,v in g.items())]
def _stock_rows(selected):
    rows=[]
    for i,(_,r) in enumerate(selected.iterrows(),1):rows.append([i,r.get("Symbol","-"),r.get("PriceBucket","-"),format_money(r.get("Current_Price",0)),format_money(r.get("Current_Open",0)),format_money(r.get("Current_High",0)),format_money(r.get("Current_Low",0)),format_money(r.get("Current_Close",r.get("Current_Price",0))),f"{float(r.get('Current_Volume',0)):,.0f}",f"{float(r.get('Pred_Close',0)):.2f}",f"{float(r.get('Confidence',0)):.0f}%",str(r.get("Action",r.get("Direction","-")))])
    return rows
def _scan_lines(scan):return [f"🔎 Scan funnel: Universe {int(scan.get('Universe',0)):,} → Data {int(scan.get('Data',0)):,} → Liquid {int(scan.get('Liquid',0)):,} → AI {int(scan.get('AI',0)):,} → Selected {int(scan.get('Selected',0)):,}"]
def _portfolio_lines(p):
    if not p:return ["💼 Portfolio Manager: no portfolio CSV / no verified prices."]
    lines=[f"💼 *PORTFOLIO MANAGER* | Positions {p.get('Positions',0)} | Value ₹{p.get('Value',0):,.0f} | P&L ₹{p.get('PnL',0):+,.0f} ({p.get('Return',0):+.2f}%)"]
    lines += [f"• {x}" for x in p.get("Rows",[])];return lines
...
def morning_report(prediction_date,cutoff_date,selected,jump_watchlist,intraday,**kwargs):
    ...
    lines=["📈 *AI NSE MORNING REPORT*",f"_{prediction_date} | Data: {cutoff_date} | {MODEL_VERSION}_"]+_scan_lines(scan)+_accuracy_lines(accuracy)+["","🎯 *1. TOP STOCKS*"]+_market_lines(snapshot,regime)+_sector_lines(selected)+["","*Price buckets — current OHLCV + AI forecast*"]
    ...
    if intraday is not None and not intraday.empty:
        rows=[[i,r.get("Symbol","-"),r.get("Bias","-"),format_money(r.get("Current",0)),format_money(r.get("Target",0)),format_money(r.get("StopLoss",0)),f"{float(r.get('Confidence',0)):.0f}%"] for i,(_,r) in enumerate(intraday.iterrows(),1)];lines += ["```",_transpose_table(["#","Stock","Bias","CMP","Target","SL","Conf"],rows),"```"]
...
def evening_report(market_date,evaluation,metrics,retraining,**kwargs):
    ...
    lines += _portfolio_lines(portfolio)+["","🧠 *3. STAGE 10 LEARNING*"]
