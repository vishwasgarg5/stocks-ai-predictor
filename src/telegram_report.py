...
def _accuracy_lines(a, compact=False):
    ...
    return [
        main,
        f"📏 Horizons: 1D {_fmt_accuracy(a.get('1D'))} | 3D {_fmt_accuracy(a.get('3D'))} | 5D {_fmt_accuracy(a.get('5D'))} | 7D {_fmt_accuracy(a.get('7D'))} | 20D {_fmt_accuracy(a.get('20D'))} | Direction {_fmt_accuracy(a.get('DirectionAccuracy'))}",
...
def _stock_rows(selected):
    ...
    for i, (_, r) in enumerate(selected.iterrows(), 1):
        ...
        try:
            ...
            pass
        rows.append([
            i, r.get("Symbol", "-"), r.get("PriceBucket", "-"), format_money(cmp),
            format_money(r.get("Current_Open", 0)), format_money(r.get("Current_High", 0)),
            format_money(r.get("Current_Low", 0)), format_money(r.get("Current_Close", cmp)),
            f"{float(r.get('Current_Volume', 0) or 0):,.0f}", f"{pred:.2f}",
            f"{expected:+.1f}%", sl_text, rr, f"{float(r.get('Confidence', 0) or 0):.0f}%",
            str(r.get("Action", r.get("Direction", "-")))
        ])
    return rows
...
def morning_report(prediction_date, cutoff_date, selected, jump_watchlist, intraday, **kwargs):
    ...
    lines = [
        ...
        f"_{prediction_date} | Data: {cutoff_date} | {MODEL_VERSION}_",
    ] + _scan_lines(scan) + _accuracy_lines(accuracy, compact=True) + ["", "🎯 *1. TOP STOCKS*"] + _market_lines(snapshot, regime) + _sector_lines(selected) + ["", "*Current OHLCV + AI forecast*"]

    if selected is not None and not selected.empty:
        lines += [
            "```",
            _transpose_table(["#", "Stock", "Bucket", "CMP", "Open", "High", "Low", "Close", "Volume", "AI Close", "Exp%", "SL", "R/R", "Conf", "Action"], _stock_rows(selected)),
            "```",
        ]
    else:
    ...
    if jump_watchlist is not None and not jump_watchlist.empty:
        ...
        for i, (_, r) in enumerate(jump_watchlist.head(5).iterrows(), 1):
            ...
            rows.append([i, r["Symbol"], format_money(cp), format_money(target), format_percent((target / cp - 1) * 100 if cp else 0), format_percent(float(r.get("Estimated_7D_Upside", 0))), f"{float(r.get('Jump_Probability', 0)):.0f}%"])
    ...
    if intraday is not None and not intraday.empty:
        rows = [[i, r.get("Symbol", "-"), r.get("Bias", "-"), format_money(r.get("Current", 0)), format_money(r.get("Target", 0)), format_money(r.get("StopLoss", 0)), f"{float(r.get('Confidence', 0)):.0f}%"] for i, (_, r) in enumerate(intraday.head(5).iterrows(), 1)]
...
def evening_report(market_date, evaluation, metrics, retraining, **kwargs):
    ...
    lines += ["```", _table(["Metric", "Value"], [
        ...
        ["Previous Accuracy", _fmt_accuracy(accuracy.get("PreviousAccuracy"))],
        ["Current Accuracy", _fmt_accuracy(accuracy.get("CurrentAccuracy"))],
        ["Champion/Challenger", retraining.get("Decision", "-")],
        ["Model Replaced", "YES" if retraining.get("Retrained") else "NO"],
        ["Improvement", f"{retraining.get('Improvement', 0):+.2f}%"],
        ["Learning State", learning.get("status", "UPDATED")],
    ]), "```"]
    return "\n".join(lines)
