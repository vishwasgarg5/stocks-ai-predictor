...
def _evaluation_components(row, regime):
    return {
        ...
        "Expected Return": expected_return_score(row.get("Expected_Return", 0)),
        "Model Confidence": float(row.get("Confidence", 50)),
...
def morning_report(prediction_date, cutoff_date, schedule_status, regime, model_variant, selected, jump_watchlist, intraday, universe_count=None, liquid_count=None, prescreen_count=None):
    lines = [
        ...
        _table(["Stage", "Stocks"], [
            ...
            ["ML Final Top 5", 5 if selected is not None and not selected.empty else 0],
    ...
    if jump_watchlist is not None and not jump_watchlist.empty:
        ...
        for i, (_, row) in enumerate(jump_watchlist.iterrows(), 1):
            current_price = float(row["Current_Price"]); target_level = float(row["Target_Level"])
            target_upside = ((target_level / current_price) - 1) * 100 if current_price else 0.0
            expected_7d = float(row.get("Estimated_7D_Upside", 0))
            rows.append([i, row["Symbol"], format_money(current_price), format_money(target_level), format_percent(target_upside), format_percent(expected_7d), f"{row['Jump_Probability']:.0f}%"])
        lines += ["```", _table(["#", "Stock", "CMP", "Target", "Target%", "7D Exp%", "Prob"], rows), "```", "```", _table(["Meaning", "Definition"], [["Target%", "Distance from CMP to +5% target"], ["7D Exp%", "Model-estimated 7-trading-day upside"], ["Prob", "Model probability of reaching +5% within 7 sessions"]]), "```"]
    else:
    ...
    if intraday is not None and not intraday.empty:
        ...
        for i, (_, row) in enumerate(intraday.iterrows(), 1):
            rows.append([i, row["Symbol"], row["Bias"], format_money(row["Current"]), format_money(row["Target"]), format_money(row["StopLoss"]), f"{row['Confidence']:.0f}%"])
...
def evening_report(market_date, evaluation, metrics, retraining):
    ...
    if evaluation is not None and not evaluation.empty:
        ...
        for _, row in evaluation.iterrows():
            rows.extend([[row["Symbol"], "PRED", f"{row['Pred_Open']:.2f}", f"{row['Pred_High']:.2f}", f"{row['Pred_Low']:.2f}", f"{row['Pred_Close']:.2f}"], ["", "ACT", f"{row['Actual_Open']:.2f}", f"{row['Actual_High']:.2f}", f"{row['Actual_Low']:.2f}", f"{row['Actual_Close']:.2f}"], ["", "DIFF", f"{row['Diff_Open']:+.2f}", f"{row['Diff_High']:+.2f}", f"{row['Diff_Low']:+.2f}", f"{row['Diff_Close']:+.2f}"]])
            direction_rows.append([row["Symbol"], f"{row['Pred_Direction']} → {row['Actual_Direction']}", "YES" if row["DirectionCorrect"] else "NO"])
