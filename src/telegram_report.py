...
def _accuracy_lines(accuracy):
    previous=accuracy.get("PreviousAccuracy")
    current=accuracy.get("CurrentAccuracy")
    samples=accuracy.get("AccuracySamples",accuracy.get("Samples",0))
    def fmt(v): return "-" if v is None or pd.isna(v) else f"{float(v):.1f}%"
    return [f"🤖 Model accuracy: Previous {fmt(previous)} → Current {fmt(current)} | Validated samples {int(samples or 0)}"]
...
def morning_report(prediction_date,cutoff_date,selected,jump_watchlist,intraday,**kwargs):
    ...
    scan_count=kwargs.get("scan_count",kwargs.get("scanned_stocks",0))
    lines=["📈 *AI NSE MORNING REPORT*",f"_{prediction_date} | Data: {cutoff_date} | {MODEL_VERSION}_","",f"🔎 Stocks scanned: *{int(scan_count or 0):,}*"]
    lines += _accuracy_lines(accuracy)
    lines += ["","🎯 *1. TOP STOCKS*"]
    lines += _market_lines(snapshot,regime)+["","*Price buckets — current OHLCV + AI forecast*"]
...
def evening_report(market_date,evaluation,metrics,retraining,**kwargs):
    ...
    accuracy=kwargs.get("accuracy",{})
    scan_count=kwargs.get("scan_count",kwargs.get("scanned_stocks",0))
    lines=["🌙 *AI NSE EVENING REPORT*",f"_{market_date} | {MODEL_VERSION}_","",f"🔎 Stocks scanned: *{int(scan_count or 0):,}*"]
    lines += _accuracy_lines(accuracy)
    ...
    lines += ["```",_table(["Metric","Value"],[["Samples",metrics.get("Samples",0)],["Overall MAPE",f"{metrics.get('OverallMAPE',0):.3f}%"],["Close MAPE",f"{metrics.get('CloseMAPE',0):.3f}%"],["Direction Accuracy",f"{metrics.get('DirectionAccuracy',0):.1f}%"],["Previous Accuracy",("-" if accuracy.get('PreviousAccuracy') is None else f"{float(accuracy.get('PreviousAccuracy')):.1f}%")],["Current Accuracy",("-" if accuracy.get('CurrentAccuracy') is None else f"{float(accuracy.get('CurrentAccuracy')):.1f}%")],["Champion/Challenger",retraining.get("Decision","-")],["Model Replaced","YES" if retraining.get("Retrained") else "NO"],["Improvement",f"{retraining.get('Improvement',0):+.2f}%"],["Learning State",learning.get("status","UPDATED")]]) ,"```"]
    return "\n".join(lines)
