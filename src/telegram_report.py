...
def _post_telegram(message,token,chat_id):
    ...
    except Exception as exc:print("Telegram plain-text exception:",exc);return False
...
def _split_safe(text,max_length):
    ...
    for line in text.splitlines(True):
        ...
        if current and len(current)+len(line)>max_length:
            if in_code:current+="```\n"
            parts.append(current.rstrip());current="```\n" if in_code else ""
        current+=line
...
def _bucket_sections(selected):
    ...
    for bucket in _ordered_price_buckets(selected["PriceBucket"]):
        ...
        for _,r in _sort(g).head(6).iterrows():
            ...
            rows.append([r.get("Symbol","-"),f"₹{_fmt(cp)}",_pct(exp),_pct(r.get("Horizon_1D")),_pct(r.get("Horizon_5D")),_pct(r.get("Horizon_20D")),_decision(r.get("Action"))]);shown+=1
...
def _best_pick_table(selected):
    ...
    for bucket in _ordered_price_buckets(selected["PriceBucket"]):
        ...
        rows.append([f"₹{_bucket_label(bucket,g)}",r.get("Symbol","-"),f"₹{_fmt(r.get('Current_Price',r.get('Current_Close')))}",_pct(r.get("Expected_Return")),"-" if score is None else f"{score:.0f}",_decision(r.get("Action"))])
...
def _prediction_table(selected):
    n=0 if selected is None else len(selected)
    if n==0:return ["📈 *PREDICTED OHLC — TOP 0*","No predictions available."]
    n=min(10,n);rows=[[r.get("Symbol","-"),_fmt(r.get("Pred_Open")),_fmt(r.get("Pred_High")),_fmt(r.get("Pred_Low")),_fmt(r.get("Pred_Close"))] for _,r in _sort(selected).head(n).iterrows()]
...
def _horizon_table(selected):
    ...
    for _,r in _sort(selected).head(n).iterrows():rows.append([r.get("Symbol","-")]+[_pct(_horizon_value(r,h)) for h in REPORT_HORIZONS]+[_horizon_status(r)])
...
def _jump(j):
    ...
    x=j.copy();x["_jump_sort"]=pd.to_numeric(x.get("Jump_Probability"),errors="coerce") if "Jump_Probability" in x.columns else 0;x=x.sort_values("_jump_sort",ascending=False,kind="mergesort");rows=[]
    for _,r in x.iterrows():
        ...
        rows.append([r.get("Symbol","-"),f"₹{_fmt(cp)}",f"₹{_fmt(target)}",_pct((target/cp-1)*100),f"{max(0,min(100,prob or 0)):.0f}%"])
        if len(rows)==5:break
    return ["🔥 *JUMP WATCH — TOP 5*",*_table(["Stock","CMP","Target","Upside","Prob"],rows)] if rows else ["🔥 *JUMP WATCH — TOP 5*","No valid jump candidates."]

def _intraday(x):
    if x is None or x.empty:return ["⚡ *INTRADAY TOP 5*","No qualifying intraday setup. Live feed may be unavailable, stale, or all candidates failed quality gates."]
    rows=[]
    for _,r in x.head(5).iterrows():rows.append([r.get("Symbol","-"),r.get("Status",_decision(r.get("Bias"))),_decision(r.get("Bias")),f"₹{_fmt(r.get('Current'))}",f"₹{_fmt(r.get('Target'))}",f"₹{_fmt(r.get('StopLoss'))}",f"{max(0,min(100,_num(r.get('Confidence'),0) or 0)):.0f}%"])
...
def _portfolio(p):
    ...
    for x in p.get("Rows",[]):
        ...
        rows.append([x.get("Stock","-"),x.get("Quantity","-"),_decision(x.get("Decision")),x.get("Current_Price","-"),x.get("Average_Price","-"),x.get("AI_Target","-"),x.get("Return_Pct","-"),x.get("Profit_Target","-"),_portfolio_horizon_status(x),x.get("Sell_Window","-")])
    return lines+(_table(["Stock","Qty","Decision","CMP","Avg","AI Target","Return","10% Target","Horizon","Sell Window"],rows,max_width=14) if rows else ["Portfolio positions exist, but no renderable rows were produced."])
