"""Stage 10.4 mobile-first Telegram reports."""
import os
import requests
import pandas as pd
...
def _stock_rows(group):
    ...
    for _,r in group.iterrows():
        cmp=float(r.get("Current_Price",r.get("Current_Close",0)) or 0);pred=float(r.get("Pred_Close",0) or 0);exp=((pred/cmp)-1)*100 if cmp else 0;score=r.get("FinalDecisionScore",r.get("FinalScore",r.get("Score",r.get("TradeConfidence",0))))
...
def _prediction_table(selected):
    ...
    rows=[]
    for _,r in selected.iterrows():rows.append([f"*{r.get('Symbol','-')}*",_fmt(r.get("Pred_Open")),_fmt(r.get("Pred_High")),_fmt(r.get("Pred_Low")),_fmt(r.get("Pred_Close")),_fmt(r.get("Pred_Volume"),0),_pct(r.get("Expected_Return"))])
    return ["📈 *PREDICTED OHLCV*"]+_table(["Stock","Open","High","Low","Close","Volume","Exp%"],rows)

def _horizon_table(selected):
    if selected is None or selected.empty:return []
    rows=[]
    for _,r in selected.iterrows():rows.append([f"*{r.get('Symbol','-')}*"]+[_pct(r.get(f"Horizon_{h}D")) for h in (1,3,5,7,20)])
...
def _ipo(ipo):
    ...
    rows=[]
    for _,r in ipo.iterrows():rows.append([f"*{r.get('IPOName','-')}*",f"₹{_fmt(r.get('PriceHigh',0),0)}",f"₹{_fmt(r.get('GMPValue',0),0)}",_pct(r.get('GMPPct',0)),_fmt(r.get('IPOScore',0),0),r.get('IPOAction','WATCH')])
...
def _jump(jump_watchlist):
    ...
    rows=[]
    for _,r in jump_watchlist.iterrows():
...
def _intraday(intraday):
    ...
    rows=[]
    for _,r in intraday.iterrows():rows.append([f"*{r.get('Symbol','-')}*",r.get('Bias','-'),f"₹{_fmt(r.get('Current'))}",f"₹{_fmt(r.get('Target'))}",f"₹{_fmt(r.get('StopLoss'))}",f"{float(r.get('Confidence',0) or 0):.0f}%"])
...
def morning_report(prediction_date,cutoff_date,selected,jump_watchlist,intraday,**kwargs):
    ...
    lines += _bucket_sections(selected)+[""]+_best_pick_table(selected)+[""]+_prediction_table(selected)+[""]+_horizon_table(selected)+[""]+_jump(jump_watchlist)+[""]+_intraday(intraday)+[""]+_ipo(ipo)+[""]+_portfolio(portfolio)
    return "\n".join(lines)
...
def _evening_accuracy_table(bucket):
    ...
    return _table(["Price Bucket","Accuracy"],[[k,_accuracy(v)] for k,v in bucket.items()])

def _evening_horizon_table(horizon):
    if not horizon:return ["No horizon target matured yet"]
    return _table(["Horizon","Accuracy","Samples"],[[f"{k}D",f"{v.get('Accuracy',0):.1f}%",v.get('Samples',0)] for k,v in sorted(horizon.items(),key=lambda x:int(x[0]))])
