...
def send_telegram(text):
    ...
    url=f"https://api.telegram.org/bot{token}/sendMessage";parts=text.split(_BUCKET_MESSAGE) if _BUCKET_MESSAGE in text else [text];success=True
    for part in parts:
...
def _horizon_lines(r):
    cmp=float(r.get("Current_Price",r.get("Current_Close",0)) or 0);targets=[];returns=[]
    for h in (1,3,5,7,20):
        try:ret=float(r.get(f"Horizon_{h}D"));targets.append(f"₹{_fmt(cmp*(1+ret/100)) if cmp else '-'}");returns.append(_pct(ret))
        except (TypeError,ValueError):targets.append("-");returns.append("-")
    return ["Horizon | 1D | 3D | 5D | 7D | 20D","Target  | "+" | ".join(targets),"Exp%    | "+" | ".join(returns)]
def _ohlcv_lines(r,cmp):return ["OHLCV | Open | High | Low | Close | Volume","      | "+" | ".join([_fmt(r.get("Current_Open")),_fmt(r.get("Current_High")),_fmt(r.get("Current_Low")),_fmt(r.get("Current_Close",cmp)),_fmt(r.get("Current_Volume"),0)])]
def _stock_card(r,warning=False):
    cmp=float(r.get("Current_Price",r.get("Current_Close",0)) or 0);pred=float(r.get("Pred_Close",0) or 0);expected=((pred/cmp)-1)*100 if cmp else 0;sl=r.get("StopLoss",r.get("SL"));sl_text=format_money(sl) if sl is not None and not pd.isna(sl) else "-";rr="-"
    try:
...
def _bucket_sections(selected,warning=False):
    ...
    for bucket in buckets:
        ...
        label=str(group.iloc[0].get("PriceBucketLabel",bucket));label=bucket if label in {"nan","None","-"} else label;lines += [_BUCKET_MESSAGE,f"💎 *{label}*"]
        for _,row in group.head(5).iterrows():lines+=_stock_card(row,warning)+[""]
    return lines
def _scan(scan):return f"🔎 {int(scan.get('Universe',0)):,} scanned  •  {int(scan.get('Data',0)):,} data  •  {int(scan.get('Liquid',0)):,} liquid  •  {int(scan.get('AI',0)):,} AI  •  {int(scan.get('Selected',0)):,} selected"
def _market(snapshot,regime):
    ...
    return ["Index | NIFTY | BANKNIFTY | FINNIFTY | MIDCPNIFTY | VIX",f"Value | {_fmt(n.get('Close'))} | {_fmt(b.get('Close'))} | {_fmt(f.get('Close'))} | {_fmt(m.get('Close'))} | {_fmt(v.get('Close'))}",f"1D%   | {_pct(n.get('Change1D'))} | {_pct(b.get('Change1D'))} | {_pct(f.get('Change1D'))} | {_pct(m.get('Change1D'))} | -",f"Breadth | {int(br.get('Advancers',0))}↑ / {int(br.get('Decliners',0))}↓ | Regime {regime}"]
def _portfolio(p):
    if not p:return []
