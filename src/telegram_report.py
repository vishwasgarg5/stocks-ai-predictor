...
def _horizon_table(selected):
    if selected is None or selected.empty:return []
    rows=[[str(r.get("Symbol","-"))]+[_pct(r.get(f"Horizon_{h}D")) for h in (1,3,5,7,20)] for _,r in _sort(selected).head(10).iterrows()];return ["🔮 *MULTI-HORIZON OUTLOOK — TOP 10*"]+_table(["Stock","1D","3D","5D","7D","20D"],rows)
def _jump(j):
