        ...
        for message in split_messages(part.strip(), TELEGRAM_MAX_LENGTH):
            try:
                ...
                if r.status_code != 200:
                    print("Telegram error:", r.text); success = False
            except Exception as exc:
                print("Telegram exception:", exc); success = False
    return success
...
def _horizon_lines(r):
    cmp = float(r.get("Current_Price", r.get("Current_Close", 0)) or 0); horizons = (1, 3, 5, 7, 20); targets=[]; returns=[]
    for h in horizons:
        try:
            ret=float(r.get(f"Horizon_{h}D")); targets.append(f"₹{_fmt(cmp*(1+ret/100)) if cmp else '-'}"); returns.append(_pct(ret))
...
def _ohlcv_lines(r, cmp):
    return ["OHLCV | Open | High | Low | Close | Volume", "      | " + " | ".join([_fmt(r.get("Current_Open")), _fmt(r.get("Current_High")), _fmt(r.get("Current_Low")), _fmt(r.get("Current_Close", cmp)), _fmt(r.get("Current_Volume"), 0)])]


def _stock_card(r, warning=False):
    cmp=float(r.get("Current_Price", r.get("Current_Close", 0)) or 0); pred=float(r.get("Pred_Close", 0) or 0); expected=((pred/cmp)-1)*100 if cmp else 0
...
def morning_report(prediction_date, cutoff_date, selected, jump_watchlist, intraday, **kwargs):
    accuracy=kwargs.get("accuracy",{}); scan=kwargs.get("scan",{}); portfolio=kwargs.get("portfolio",{}); snapshot=kwargs.get("market_snapshot",{}); regime=kwargs.get("regime","-")
