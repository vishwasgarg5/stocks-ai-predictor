...
def evaluate_baseline(evaluation,predictions=None):
    ...
    default_cost=(float(TRANSACTION_COST_BPS)+float(SLIPPAGE_BPS))/100;cost=_series(x,"Baseline_Cost_Pct",default_cost).fillna(default_cost);x["CostPct"]=cost
    ...
    if not out.empty:
        existing=pd.read_csv(BASELINE_FILE) if BASELINE_FILE.exists() else pd.DataFrame();combined=pd.concat([existing,out],ignore_index=True) if not existing.empty else out;keys=[c for c in ["Prediction_ID","MarketDate","Symbol"] if c in combined.columns]
...
def _historical_validation_score(horizon):
    ...
    m=_num(h.iloc[-1].get("MAPE"),np.nan);d=_num(h.iloc[-1].get("DirectionAccuracy"),np.nan);scores=[]
...
def _market_regime_score():
    ...
    d=pd.concat(frames,ignore_index=True);direction=_series(d,"DirectionCorrect").dropna()
...
def adaptive_confidence(symbol=None,horizon=1):
    ...
    historical,hist_reason=_historical_validation_score(horizon);market_regime,regime_reason=_market_regime_score();error_score=np.clip(100-mape*10,0,100);uncertainty_score=np.clip(100-uncertainty,0,100)
...
def run_quality_controls(market_date=None):
    ...
    # Do not derive a portfolio profit target from accuracy/MAPE. The portfolio
    # layer must supply actual expected-return forecasts to confirm_portfolio_target().
    result["PortfolioTargetPolicy"]={"Status":"DEFERRED_TO_PORTFOLIO_FORECASTS","MinimumTargetPct":10.0}
