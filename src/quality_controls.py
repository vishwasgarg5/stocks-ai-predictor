...
Adds exact Prediction_ID tracking, a previous-close baseline, live horizon metrics,
adaptive confidence, and conservative long-horizon Portfolio Manager target policy.
"""
...
def evaluate_baseline(evaluation: pd.DataFrame, predictions: pd.DataFrame|None=None) -> pd.DataFrame:
    ...
    default_cost=(TRANSACTION_COST_BPS+SLIPPAGE_BPS)/100
    cost=x.get("Baseline_Cost_Pct",default_cost)
    x["CostPct"]=pd.to_numeric(cost,errors="coerce").fillna(default_cost)
...
def horizon_accuracy() -> pd.DataFrame:
    ...
    for h,g in x.groupby("HorizonDays"):
        ape=pd.to_numeric(g["APE"],errors="coerce").abs().dropna();direction=pd.to_numeric(g["DirectionCorrect"],errors="coerce").dropna()
...
def adaptive_confidence(symbol: str|None=None,horizon: int=1) -> dict:
    ...
    mape=_num(pd.to_numeric(recent.get("APE_Close",pd.Series(dtype=float)),errors="coerce").abs().mean(),10.0)
    ...
    if BASELINE_FILE.exists():
        try:
            ...
            b=b.tail(50)
            if not b.empty:baseline_edge=_num(pd.to_numeric(b["AI_Better_Than_Baseline"],errors="coerce").mean()*100,50.0)
        except Exception:pass
    error_score=max(0.0,min(100.0,100.0-mape*10.0));raw=0.35*error_score+0.25*direction+0.20*baseline_edge+0.20*50.0;evidence=min(1.0,n/30.0);confidence=50.0+(raw-50.0)*evidence
...
def confirm_portfolio_target(forecasts: dict[int,float],minimum_target: float=10.0) -> dict:
    """Require at least two confirming long horizons before accepting a >10% target."""
    clean={}
    for h,v in forecasts.items():
        value=_num(v)
        if np.isfinite(value):clean[int(h)]=value
    ...
    return {"TargetPct":round(float(minimum_target if best>=minimum_target else max(0.0,best)),2),"Status":"SINGLE_HORIZON_OR_INCONSISTENT","ConfirmedHorizons":[]}
...
def run_quality_controls(market_date: str|None=None) -> dict:
    ...
    if market_date:
        ...
        if path.exists():
            ...
            predictions=load_predictions(pd.Timestamp(prediction_date).date());base=evaluate_baseline(evaluation,predictions);result["BaselineSamples"]=len(base);result["BaselineWinRate"]=_num(base["AI_Better_Than_Baseline"].mean()*100) if not base.empty else None
    h=horizon_accuracy();result["HorizonMetrics"]=h.to_dict("records") if not h.empty else [];result["AdaptiveConfidence"]=adaptive_confidence();STATE_DIR.mkdir(parents=True,exist_ok=True);(STATE_DIR/"quality_controls.json").write_text(json.dumps(result,indent=2,default=str));return result
